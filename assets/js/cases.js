...
list()})
if(qs('#detail'))(async()=>{try{const id=getCaseId(),x=await API.record(CONFIG.tables.cases,id)
qs('#detail').innerHTML=`<div class="grid"><div class="col-4"><b>Folio</b><p>${esc(x.folio)}</p></div><div class="col-4"><b>Distrito</b><p>${esc(x.distrito)}</p></div><div class="col-4"><b>UT</b><p>${esc(x.unidad_territorial)}</p></div><div class="col-6"><b>Solicitante</b><p>${esc(x.nombre_solicitante)}</p></div><div class="col-6"><b>Clasificación</b><p>${esc(x.clasificacion)}</p></div><div class="col-12"><b>Descripción</b><p>${esc(x.descripcion)}</p></div></div>`
...
localStorage.setItem('mgpc_current_case',id)}catch(e){notify(e.message,'error')}})()
