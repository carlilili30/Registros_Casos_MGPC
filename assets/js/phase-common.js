...
initShell()
const phase=Number(document.body.dataset.phase),id=getCaseId()
...
btn.disabled=true
try{const data=formObject(e.currentTarget)
delete data.archivos
await API.create(CONFIG.tables.phases,{id_caso:Number(id),fase:phase,estatus:'CONCLUIDA',observaciones:data.observaciones||'',datos_json:JSON.stringify(data),fecha_fin:localDateTime()})
...
for(const f of up.subidos||[])await API.create(CONFIG.tables.caseFiles,{id_caso:Number(id),id_archivo:Number(f.id),fase:phase,nombre_original:f.nombre_original})}const c=await API.record(CONFIG.tables.cases,id)
if(phase===8){await API.update(CONFIG.tables.cases,id,{fase_actual:8,estatus:'CONCLUIDO'})
location.href=`../casos/detalle.html?id=${id}`
return}let next=phase+1
...
location.href=`fase${next}-${slugs[next]}.html?id=${id}`}catch(x){notify(x.message,'error')
btn.disabled=false}})
load()
